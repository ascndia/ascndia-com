...
const SeedSkill = async (payload: Payload, param: Omit<Skill, 'id' | 'createdAt' | 'updatedAt'>): Promise<void> => {
  await payload.create({
    collection: 'skills',
    data: param
  });
  payload.logger.info(`Seeding skill: ${param.title}`);
};

// Function to seed all skills
const SeedSkills = async (payload: Payload): Promise<void> => {
